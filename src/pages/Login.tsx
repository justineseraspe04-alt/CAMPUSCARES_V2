...
const ROLES = [
  { id: 'recipient', label: 'Recipient', icon: GraduationCapIcon, color: 'cyan' },
  { id: 'donor', label: 'Donor', icon: HeartHandshakeIcon, color: 'emerald' },
  { id: 'admin', label: 'Admin', icon: ShieldIcon, color: 'sky' },
] as const;
...
export function Login() {
  ...
  useEffect(() => {
    ...
    if (roleParam === 'admin' || roleParam === 'donor' || roleParam === 'recipient') {
      setRole(roleParam);
    }
  }, [searchParams]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus('success');
      const destination =
        redirectFrom && redirectFrom !== '/login' && redirectFrom !== '/register'
          ? redirectFrom
        ...
        <div className="max-w-md w-full mx-auto">
          ...
          <motion.div
            ...
            <p className="text-slate-600 mb-8">
              Sign in to access your dashboard and continue supporting the campus community.
            </p>
            ...
            {forgotHint && (
              <div className="mb-6 p-4 rounded-xl bg-sky-50 border border-sky-100 text-sm text-sky-800">
                Password reset is not available yet. Contact your campus administrator for help.
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder={role === 'admin' ? 'Enter admin123' : '••••••••'}
                    disabled={isSubmitting}
              ...
              {role === 'admin' && (
                <div className="p-3 rounded-lg bg-slate-100 border border-slate-200 text-xs text-slate-600 flex items-start gap-2">
                  ...
                  <p>
                    Admin accounts are managed by the system. Use <strong>admin</strong> /{' '}
                    <strong>admin123</strong> to test.
                  </p>
                </div>
              )}
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="w-full py-3.5 px-4 rounded-xl bg-sky-600 text-white font-medium hover:bg-sky-700 transition-colors shadow-sm shadow-sky-200 mt-2 disabled:opacity-60">
        ...
        <div className="relative z-10 w-full max-w-md">
          <motion.div
            ...
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="text-lg font-bold text-slate-800">Campus Inventory</h3>
            ...
            <div className="space-y-4">
              {[
                { icon: BookIcon, name: 'Calculus Textbook', status: 'Just Added', color: 'bg-blue-100 text-blue-600' },
                { icon: ShirtIcon, name: 'Campus Jacket', status: 'Available', color: 'bg-emerald-100 text-emerald-600' },
                { icon: PackageIcon, name: 'Lab Equipment', status: 'Requested', color: 'bg-amber-100 text-amber-600' },
              ].map((item, i) => (
                <div
                  key={i}
            ...
            <div className="mt-8 p-4 rounded-2xl bg-sky-600 text-white flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-sky-200">making a difference today</p>
              </div>
            </div>
          </motion.div>
        </div>
