...
import { mapCategoryToApi, mapConditionToApi } from '../../utils/donationDisplay';
...
export function Donate() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!condition.trim()) {
      setMessage({ type: 'error', text: 'Condition is required.' });
      return;
    }
    ...
    const payload: donorApi.DonationPayload = {
      donorName: user.fullName,
      donorEmail: user.email,
      itemName: itemName.trim(),
      category: mapCategoryToApi(category),
      itemCondition: mapConditionToApi(condition),
      quantity,
      size: size.trim() || undefined,
      subjectOrCourse: subjectOrCourse.trim() || undefined,
    ...
    try {
      ...
      setMessage({
        type: 'success',
        text: 'Donation submitted successfully and is now pending admin approval.',
      });
      setItemName('');
      ...
      setMessage({
        type: 'error',
        text: err instanceof Error ? err.message : 'Failed to submit donation.',
      });
    } finally {
      setLoading(false);
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <motion.div
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 px-4 rounded-xl bg-emerald-600 text-white font-medium hover:bg-emerald-700 transition-colors shadow-sm text-lg disabled:opacity-60">
        ...
        <motion.div
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-lg font-bold text-slate-800">Donation Guidelines</h3>
          </div>
