...
export function Request() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!requestedItemName || requestedItemName.trim() === '') {
    ...
    try {
      setLoading(true);
      await requestApi.submitRequest(payload);
      setMessage({type:'success', text:'Request submitted successfully and is now pending admin review.'});
      setRequestedItemName(''); setReason(''); setCategory('Books');
  ...
  return (
    <PlaceholderPage
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} className="lg:col-span-2 bg-white rounded-3xl p-6 md:p-8 border border-slate-200 shadow-sm">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <input type="text" value={requestedItemName} onChange={e=>setRequestedItemName(e.target.value)} placeholder="e.g. Scientific Calculator" className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white focus:outline-none" />
            </div>

            <div className="space-y-2">
              <label className="block text-sm font-medium text-slate-700">Category</label>
              <select value={category} onChange={e=>setCategory(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white focus:outline-none text-slate-700">
                <option>Books</option>
            ...
            <div className="space-y-2">
              ...
              <textarea rows={4} value={reason} onChange={e=>setReason(e.target.value)} placeholder="Briefly explain how this item will help you..." className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white focus:outline-none resize-none"></textarea>
            </div>

            {message && (<div className={`p-3 rounded ${message.type==='success' ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : 'bg-rose-50 border-rose-100 text-rose-700'}`}>{message.text}</div>)}

            <button type="submit" disabled={loading} className="w-full py-3.5 px-4 rounded-xl bg-cyan-600 text-white font-medium hover:bg-cyan-700 transition-colors shadow-sm text-lg">{loading ? 'Submitting...' : 'Submit Request'}</button>
          </form>
        </motion.div>
        ...
        <motion.div
          ...
          <ul className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 before:to-transparent">
            {[
            {
              ...
              desc: 'Fill out the form with your needs.'
            },
            {
              title: 'Admin reviews',
              desc: 'We match your request with inventory.'
            },
            {
              title: 'Pickup',
              desc: 'Collect your item at the Student Center.'
            }].
            map((step, i) =>
